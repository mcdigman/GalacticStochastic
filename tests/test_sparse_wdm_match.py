...
def test_ts_match_TT_TS(fdot_mult: float) -> None:
    ...
    assert_allclose(waveTT[maskTTTS], waveTS[maskTTTS], atol=1.0e-2, rtol=1.0e-10)

    pow_TT = np.sum(waveTT**2)
    pow_TS = np.sum(waveTS**2)
    # check powers match

    assert_allclose(pow_TT, pow_TS, atol=1.0e-11, rtol=7.0e-4)
...
def test_Chirp_wdm_match_TT_TTexact(fdot_mult: float) -> None:
    ...
    assert_allclose(waveTT, waveTT_exact, atol=3.0e-4, rtol=1.0e-15)
...
def test_Chirp_wdm_match_TT_long(fdot_mult: float) -> None:
    ...
    nt_lim_waveform = PixelGenericRange(0, wc.Nt, wc.DT, 0.0)
    nt_lim_waveform_long = PixelGenericRange(0, wc.Nt * wc.Nf, wc.dt, 0.0)
    ...
    waveform_long_c = StationaryWaveformTime(
        T_long, np.zeros(n_long), np.zeros(n_long), np.zeros(n_long), np.zeros(n_long)
    )
    ...
    n_fft = fs_fft.size
    waveform_long_f = StationaryWaveformFreq(fs_fft, np.zeros(n_fft), np.zeros(n_fft), np.zeros(n_fft), np.zeros(n_fft))
    ...
    assert_allclose(wave_got_freq, wave_got_time, atol=3.0e-4, rtol=1.0e-15)

    assert_allclose(waveTT[(waveTT != 0.0)], wave_got_freq[(waveTT != 0.0)], atol=2.0e-2, rtol=1.0e-10)
    assert_allclose(waveTT[(waveTT != 0.0)], wave_got_time[(waveTT != 0.0)], atol=2.0e-2, rtol=1.0e-10)

    pow_freq = np.sum(wave_got_freq**2)
    pow_time = np.sum(wave_got_time**2)
    pow_TT = np.sum(waveTT**2)
    # check powers match
    assert_allclose(pow_freq, pow_time, atol=1.0e-11, rtol=1.0e-9)
    assert_allclose(pow_freq, pow_TT, atol=1.0e-11, rtol=7.0e-4)
    ...
    assert matchsigf_sigt < 1.0e-6
    assert matchTTsigt < 1.0e-3

    assert matchTTsigf < 1.0e-3
...
def test_Chirp_wdm_match_TS_long(fdot_mult: float) -> None:
    ...
    assert_allclose(waveTS[(waveTS != 0.0)], wave_got_freq[(waveTS != 0.0)], atol=2.0e-2, rtol=1.0e-10)
    assert_allclose(waveTS[(waveTS != 0.0)], wave_got_time[(waveTS != 0.0)], atol=2.0e-2, rtol=1.0e-10)
    ...
    # check powers match
    assert_allclose(pow_freq, pow_time, atol=1.0e-11, rtol=1.0e-9)
    assert_allclose(pow_freq, pow_TS, atol=1.0e-11, rtol=6.0e-5)
    ...
    assert matchsigf_sigt < 1.0e-6
    assert matchTSsigt < 1.0e-3

    assert matchTSsigf < 1.0e-3
