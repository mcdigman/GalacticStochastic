...
def TaylorF2_inplace(intrinsic_waveform: StationaryWaveformFreq, params_intrinsic: BinaryIntrinsicParams, nf_lim: PixelGenericRange, t_offset: float = 0., tc_mode: int = 0) -> float:
    ...
    nu_lso: float = (np.pi * Mt * f_lso)**(1 / 3)
    ...
    if tc_mode:
        ...
        TTRef = p0 / nu_i**8 * (1 + p1 * nu_i**2 + p2 * nu_i**3 + p3 * nu_i**4 + p4 * nu_i**5 + (6848 / 105 * np.log(4 * nu_i) - 712192 / 105 + p5) * nu_i**6 + p6 * nu_i**7) + t_offset
    psi_i: float = 2 * np.pi * TTRef * f_i - np.pi / 4 + c0 / nu_i**5 * (1 + c1 * nu_i**2 + c2 * nu_i**3 + c3 * nu_i**4 + c4 * (1 + 3 * np.log(nu_i / nu_lso)) * nu_i**5 + (c5 - 6848 / 21 * np.log(4 * nu_i)) * nu_i**6 + c6 * nu_i**7)
    psi_ref: float = -2 * phic - psi_i - np.pi / 4

    Amp: float = np.sqrt(5 / 6) / np.pi**(2 / 3) * Mc**(5 / 6) / np.exp(log_dl)
    for n in range(nf_lim.nx_min, nf_lim.nx_max):
        nu: float = (np.pi * Mt * FS[n])**(1 / 3)
        m4: float = 1 + 3 * np.log(nu / nu_lso)
...
def TaylorF2_eccentric_inplace(intrinsic_waveform: StationaryWaveformFreq, params_intrinsic: BinaryIntrinsicParams, nf_lim: PixelGenericRange, amplitude_pn: int = 0, t_offset: float = 0., tc_mode: int = 0) -> float:
    ...
    ay50: float = -(3317 * 2 / 252) * np.log(4 * nu_i)
    ...
    m4_i: float = 1 + 3 * np.log(nu_i)
    ac5_i: float = -(3424 / 21) * 2 * np.log(4 * nu_i)
    ap5_i: float = (3424 / 105) * 2 * np.log(4 * nu_i) - 712192 / 105
    ay05I: float = (536803271 / 39564000) * 2 * np.log(4 * nu_i)
...
def TaylorF2_eccentricity_solve(params_intrinsic: BinaryIntrinsicParams, f_i2: float) -> float:
    ...
    nu_i1: float = (np.pi * Mt * f_i)**(1 / 3)
    nu_i2: float = (np.pi * Mt * f_i2)**(1 / 3)
    ...
    ddy0_derived: float = ddy0 * (nu_i1 / nu_i2)**(19 / 3) *\
          (
            1
            + ddy10 * nu_i1**2
            + ddy20 * nu_i1**3
...
def TaylorF2_eccentric_TTRef(params_intrinsic: BinaryIntrinsicParams) -> float:
    ...
    # starting values of coefficients that change in the loop
    ap5: float = (3424 / 105) * 2 * np.log(4 * nu_i) - 712192 / 105
    ay05: float = (536803271 / 39564000) * 2 * np.log(4 * nu_i)
    day05 = 16 / 34 * ay05 - 31576663 / 13188000
...
def TaylorF2_aligned_inplace(intrinsic_waveform: StationaryWaveformFreq, params_intrinsic: BinaryIntrinsicParams, nf_lim: PixelGenericRange, *, include_phenom_amp: int = 1, include_pn_ss3: int = 0, t_offset: float = 0., tc_mode: int = 0) -> float:
    ...
    a0: float = 1 / 2 * np.sqrt(5 / 6) / np.pi**(2 / 3) * Mc**(5 / 6) / np.exp(log_dl) * (np.pi * Mt)**(7 / 6)
    ...
    nu_i: float = (np.pi * Mt * f_i)**(1 / 3)
    ac5_i: float = -6848 / 21 * np.log(4 * nu_i)
    ap5_i: float = 6848 / 105 * np.log(4 * nu_i) - 712192 / 105
    m4_i: float = 1 + 3 * np.log(nu_i / nu_lso)
    ...
    for n in range(nf_lim.nx_min, nf_lim.nx_max):
        ...
        m4: float = 1 - 3 * np.log(nu_lso) + 3 * np.log(nu)
